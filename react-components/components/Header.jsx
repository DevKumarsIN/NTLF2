...
const Header = () => {
  ...
  useEffect(() => {
    ...
    // --- Dynamic text color for fixed button over dark sections ---
    const fixedBtn = document.getElementById('fixedRoundBtn');
    const darkSections = document.querySelectorAll('.dark-section');
    let observer = null;
    if (fixedBtn && darkSections.length > 0) {
      observer = new window.IntersectionObserver((entries) => {
        ...
        if (isOverDark) {
          fixedBtn.classList.add('dark-bg');
        } else {
          fixedBtn.classList.remove('dark-bg');
        }
      }, {
      ...
      >
        ...
        <span className="rotating-texts-svg">
          <svg width="100" height="100" viewBox="0 0 100 100"
            ...
            <text fontSize="11" fontWeight="600" textLength="400" lengthAdjust="spacingAndGlyphs">
              <textPath xlinkHref="#circlePath" startOffset="0">
                <tspan className="circle-text-dark">REGISTER NOW. REGISTER NOW. REGISTER NOW. REGISTER NOW.</tspan>
                <tspan className="circle-text-light">REGISTER NOW. REGISTER NOW. REGISTER NOW. REGISTER NOW.</tspan>
              </textPath>
            </text>
      <style>{`
        .circle-text-dark {
        ...
        .fixed-round-btn.dark-bg .circle-text-dark {
          display: none;
        }
        .fixed-round-btn.dark-bg .circle-text-light {
          display: inline;
        }
      `}</style>
          </svg>
